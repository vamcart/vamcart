...
foreach ($modules AS $module)
{
	...
	{
		$action_button = $this->Admin->linkButton(__('Install'),'/' . $module['alias'] . '/setup/install/','install.png', array('escape' => false, 'class' => 'button'));
	}
	elseif((isset($module['installed_version']))&&($module['installed_version'] < $module['version']))
	{
		$action_button = $this->Admin->linkButton(__('Upgrade'),'/' . $module['alias'] . '/setup/upgrade/','upgrade.png', array('escape' => false, 'class' => 'button'),__('Are you sure?'));
	}
	else
	{
		$action_button = $this->Admin->linkButton(__('Uninstall'),'/' . $module['alias'] . '/setup/uninstall/','uninstall.png', array('escape' => false, 'class' => 'button'),__('Are you sure?'));
	}

	echo $this->Admin->TableCells(
		  array(
		  	$this->Html->link($module['name'],'/' . $module['alias'] . '/admin/admin_help'),
