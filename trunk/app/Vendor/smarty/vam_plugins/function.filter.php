	...
	<h3 class="widget-title">{lang}Filter{/lang}</h3>
		<form class="form-horizontal" name="filter" action="{$base_url}/filtered/set/{$base_content}" method="post">
			<div class="filter">
				...
				{/foreach}
				<button class="btn btn-default" name="applybutton" type="submit"><i class="fa fa-check"></i> {lang}Apply{/lang}</button>
				<button class="btn btn-default" name="cancelbutton" type="submit"><i class="fa fa-times"></i> {lang}Reset{/lang}</button>
			</div>
		</form>
</section>
';
