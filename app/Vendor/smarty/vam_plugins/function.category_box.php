...
function smarty_function_category_box($params, $template)
{
	global $content, $config;

	// Cache the output.
	$cache_name = 'vam_category_box_output' . (isset($params['template'])?$params['template']:'') . '_' . $_SESSION['Customer']['language_id'];
	...
	$display_template = $Smarty->load_template($params, 'category_box');
...
function smarty_help_function_category_box() {
	?>
	<h3><?php echo __('What does this tag do?') ?></h3>
	<p><?php echo __('Displays a list of categories.') ?></p>
	<h3><?php echo __('How do I use it?') ?></h3>
	<p><?php echo __('Just insert the tag into your template like:') ?> <code>{category_box}</code></p>
...
function smarty_about_function_category_box() {
}
