...
(4, 'slider', '{if $content_list}\r\n<div id="sequence-theme">\r\n	<div id="sequence">\r\n		<div class="sequence-prev"><i class="fa fa-chevron-left"></i></div>\r\n		<div class="sequence-next"><i class="fa fa-chevron-right"></i></div>\r\n			<ul class="sequence-canvas">\r\n			{foreach from=$content_list item=node}\r\n				<li>\r\n					<h2 class="title"><span><a href="{$node.url}">{$node.name|strip_tags|truncate:30:"...":true}</a></span></h2>\r\n					<div class="subtitle">					\r\n					{if $node.reviews > 0}<span class="rating">{$node.star_rating}</span> <span class="reviews">{lang}Feedback{/lang}: {$node.reviews}</span><br />{/if}\r\n                      {if $node.short_description}{$node.short_description|strip_tags|truncate:120:"...":true}<br />{/if}\r\n					<a href="{$node.url}" class="btn btn-default">{lang}read more{/lang}</a>\r\n					</div>\r\n					<img class="model" src="{$node.image}" alt="{$node.name}"{if {$node.image_width} > 0} width="{$node.image_width}"{/if}{if {$node.image_height} > 0} height="{$node.image_height}"{/if} />\r\n				</li>\r\n			{/foreach}\r\n			</ul>\r\n	</div>\r\n</div>\r\n{/if}', '2013-10-08 18:39:12', '2015-06-21 00:40:51', 'content_listing'),
