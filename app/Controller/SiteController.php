...
class SiteController extends AppController {
	...
	public function social_login($provider = "google")
	{
		...
		if ($_SESSION['fb_token'] && $_SESSION['fb_user_email'] != '' && $_SESSION['fb_user_id'] > 0) {
			// Register Customer
			$check = $this->Customer->find('first', array('order' => 'Customer.id DESC', 'conditions' => array('Customer.email' => $_SESSION['fb_user_email'])));
			if(!empty($check))
			$_POST['customer']['id']= $check['Customer']['id'];
			...
			if ($customer->validates()) {
				...
				// Customer Login
				$customer_id = $this->Customer->find('first', array('order' => 'Customer.id DESC', 'conditions' => array('email' => $_SESSION['fb_user_email'])));
