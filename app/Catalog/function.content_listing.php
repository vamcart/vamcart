...
function smarty_function_content_listing($params, $template)
{
	...
	foreach($content_list_data AS $raw_data)
	{
		...
		if(in_array(strtolower($raw_data['ContentType']['name']),$allowed_types))
		{
			...
			$content_list[$count]['is_new']	= ($raw_data['ContentProduct']['is_new'] > 0) ? $raw_data['ContentProduct']['is_new'] : false;	
			$content_list[$count]['is_featured']	= ($raw_data['ContentProduct']['is_featured'] > 0) ? $raw_data['ContentProduct']['is_featured'] : false;	
