...
class BitcoinController extends PaymentAppController {
	...
	public function before_process () 
	{
		...
		$content = '
			...
			<iframe frameborder="0" allowtransparency="true" scrolling="no" src="https://money.bitcoin.ru/embed/shop.xml?account='.$bitcoin_wallet.'&quickpay=shop&payment-type-choice=on&mobile-payment-type-choice=on&writer=seller&label='.$_SESSION['Customer']['order_id'].'&targets=Заказ №'.$_SESSION['Customer']['order_id'].'&targets-hint=&default-sum='.number_format($order['Order']['total'], 2).'&button-text=01&hint=&successURL='.$success_url.'" width="450" height="255"></iframe>
	...
	public function payment_after($order_id = 0)
	{
		...
		$content = '
			<a class="btn btn-default" href="https://money.bitcoin.ru/embed/shop.xml?account='.$bitcoin_wallet.'&quickpay=shop&payment-type-choice=on&mobile-payment-type-choice=on&writer=seller&label='.$order_id.'&targets=Заказ №'.$order_id.'&targets-hint=&default-sum='.number_format($order['Order']['total'], 2).'&button-text=01&hint=&successURL='.$success_url.'"" target="_blank" value="{lang}Pay Now{/lang}"><i class="fa fa-dollar"></i> {lang}Pay Now{/lang}</a>
			';
